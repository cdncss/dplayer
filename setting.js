    ...
    if( danmuon > 0){
	var dp = new btjson({autoplay: autoplay,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},danmaku: {id: dmid,api: dmapi,user: user}});
	}else{$('body').addClass("danmu-off");
	var dp = new btjson({autoplay: autoplay,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},});}
    // 弹出框
    //function alert_back(text) {$(".alert_back").html(text).show();setTimeout(function () { $(".alert_back").fadeOut('slow'); },1200)}
    // 通用点击
	add('.btjson-list-icon',".btjson-danmu",'show');
    ...
    $(".btjson-showing").on("click", function () {dp.play();$(".vod-pic").remove();});
    //个性化弹幕框		
	$(".btjson-comment-setting-color input").on("click", function () {
    var textcolor = $(this).attr("value"); 
    setTimeout(function (){$('.bt_json-btjson-comment-input').css({"color":textcolor});}, 100);
    });
    $(".btjson-comment-setting-type input").on("click", function () {
    var texttype = $(this).attr("value"); 
    setTimeout(function (){$('.bt_json-btjson-comment-input').attr("dmtype",texttype);}, 100);
    });
    $("#dmset").on("click", function () {
    $(".btjson-comment-icon").trigger("click");
    $(".btjson-comment-setting-box").toggleClass("btjson-comment-setting-open") 
    $("#btjson").toggleClass("btjson-hide-controller") 
    });

    $(".bt_json-btjson-send-icon").on("click", function () {
    var inputtext = document.getElementById("dmtext");
    var sendtexts = inputtext.value;
    var sendtype =$('.bt_json-btjson-comment-input').attr("dmtype");
    var sendcolor = $('.btjson-comment-input').css("color"); 
    ...
    $(".bt_json-btjson-comment-input").val("");
    })
dp.danmaku.opacity(1);
    //弹幕列表获取
	
$(".btjson-list-icon,.bt_json-btjson-send-icon").on("click", function () {		   
	$(".list-show").empty();
	$.ajax({
    url:dmapi+"?id="+dmid,
    success:function (data) {
            if (data.code == 0) {
                ...
                $(danmaku).each(function(index, item) {
                    var dammulist = `<d class="danmuku-list" time="${item[0]}"><li>${formatTime(item[0])}</li><li title="${item[4]}">${item[4]}</li><li title="用户：${item[4]}  IP地址：${item[5]}">${item[5]}</li></d>`
         ...
         $(".btjson-fulloff-icon").on("click", function () {dp.fullScreen.cancel();})
    //播放loading元素		
function video_play() {
